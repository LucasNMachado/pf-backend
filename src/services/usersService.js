...
import { UsersErrorInfo, UpdateRoleErrorInfo, UpdateRoleUserErrorInfo  } from '../utils/errors/info.js';
...
class UsersService {
  ...
  async getUserByEmail(email) {
    try {
      ...
      throw new Error('Se produjo un error al leer el correo electrónico ingresado');
    }
  }
  ...
  async createUser(bodyUser) {
    try {
      ...
      const { firstName, lastName, email, age, password, birthDate, role } = bodyUser;
  ...
  async updateUserPassword(email, newPassword) {
    try {
      ...
      const user = await this.usersRepository.getUserByEmail(email);
      if (!user) {
        throw new Error(`No se ha encontrado un usuario registrado con este correo electrónico: (${email}), verifique que los datos ingresados sean correctos y vuelva a intentarlo`);
      } else {
        const comparison = isValidPassword(user, newPassword);
        if (comparison === true) {
          throw new Error('No se puede utilizar la misma contraseña, verifique los datos');
        } else {
          const hashedPassword = createHash(newPassword);
          const updatedUser = await this.usersRepository.updateUserPassword(user._id, hashedPassword);
          return updatedUser;
        }
      }
    } catch (error) {
  ...
  async updateRole(id, newRole) {
    try {
      ...
      const user = await this.usersRepository.getUserById(id);
      if (!user) {
        throw new Error(`No se ha encontrado un usuario registrado con este ID: (${id}), verifique que los datos ingresados sean correctos y vuelva a intentarlo`);
      }
  ...
  async addDocumentUser(id, files) {
    try {
      ...
      const updatedUser = await this.usersRepository.updateUserDocuments(id, newDocuments);
      return updatedUser;
    } catch (error) {
      throw new Error(`Error al agregar documentos al usuario: ${error.message}`);
    }
  }
  ...
  async setLastConnection(user) {
    try {
      ...
      const id = user._id;
      const updatedUser = await this.usersRepository.updateLastConnection(id, new Date());
      return updatedUser;
    } catch (error) {
      throw new Error(`Error al establecer la última conexión: ${error.message}`);
    }
  }
  ...
  async deleteUserById(id) {
    try {
      ...
      if (sendMail) {
        ...
        throw new Error('Ocurrió un error en el envío del correo electrónico para notificar al usuario que su cuenta ha sido eliminada');
      }
    } catch (error) {
